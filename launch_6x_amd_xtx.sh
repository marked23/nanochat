...
# launch_6x_amd_xtx.sh - Launch Script for 6x AMD GPU Setup
# This script runs the complete nanochat training pipeline optimized for 6x 7900XTX GPUs
# It combines all activities from speedrun.sh with optimizations for AMD hardware
#
# Usage: bash launch_6x_amd_xtx.sh
# Or with wandb: WANDB_RUN=my_run bash launch_6x_amd_xtx.sh
...
AUTO_DETECT_SFT_VRAM=false          # Automatically detect VRAM and set SFT_DEVICE_BATCH_SIZE (set false to use manual value)
...
# Detect GPU VRAM and set SFT_DEVICE_BATCH_SIZE
# SFT typically uses ~60% of the VRAM compared to pretraining due to shorter sequences
detect_vram_and_set_sft_batch_size() {
    log "Detecting GPU VRAM capacity for SFT batch size..."
...
" 2>/dev/null || echo "0")
    ...
    if [ "$vram_gb" -eq 0 ]; then
        log "⚠ Warning: Could not detect VRAM, using configured SFT_DEVICE_BATCH_SIZE=$SFT_DEVICE_BATCH_SIZE"
        return
    fi
    
    log "Detected GPU VRAM: ${vram_gb}GB"
    
    # SFT batch sizes can be ~2x higher than pretraining due to variable, shorter sequences
    # Conservative values for d20 model (561M params) with BF16
    if [ "$vram_gb" -le 16 ]; then
        # 16GB: conservative for SFT
        SFT_DEVICE_BATCH_SIZE=6
        log "Setting SFT_DEVICE_BATCH_SIZE=$SFT_DEVICE_BATCH_SIZE for 16GB VRAM (conservative)"
    elif [ "$vram_gb" -le 24 ]; then
        # 24GB: can handle larger batches with shorter sequences
        SFT_DEVICE_BATCH_SIZE=12
        log "Setting SFT_DEVICE_BATCH_SIZE=$SFT_DEVICE_BATCH_SIZE for 24GB VRAM (conservative)"
    elif [ "$vram_gb" -le 32 ]; then
        # 32GB: even more headroom
        SFT_DEVICE_BATCH_SIZE=18
        log "Setting SFT_DEVICE_BATCH_SIZE=$SFT_DEVICE_BATCH_SIZE for 32GB VRAM (conservative)"
    elif [ "$vram_gb" -le 48 ]; then
        # 48GB: substantial capacity
        SFT_DEVICE_BATCH_SIZE=28
        log "Setting SFT_DEVICE_BATCH_SIZE=$SFT_DEVICE_BATCH_SIZE for 48GB VRAM (conservative)"
    elif [ "$vram_gb" -le 80 ]; then
        # 80GB (A100): large capacity
        SFT_DEVICE_BATCH_SIZE=48
        log "Setting SFT_DEVICE_BATCH_SIZE=$SFT_DEVICE_BATCH_SIZE for 80GB VRAM (conservative)"
    elif [ "$vram_gb" -le 96 ]; then
        # 96GB: very large capacity
        SFT_DEVICE_BATCH_SIZE=56
        log "Setting SFT_DEVICE_BATCH_SIZE=$SFT_DEVICE_BATCH_SIZE for 96GB VRAM (conservative)"
    else
        # 140GB+ (H100): massive capacity
        SFT_DEVICE_BATCH_SIZE=84
        log "Setting SFT_DEVICE_BATCH_SIZE=$SFT_DEVICE_BATCH_SIZE for ${vram_gb}GB VRAM (conservative)"
    fi
}
...
# =============================================================================
# ENABLED STAGES CHECK
# =============================================================================

log "Checking enabled pipeline stages..."
ENABLED_STAGES=()

# Check each stage and collect enabled ones
[ "$ENABLE_GPU_CHECK" = true ] && ENABLED_STAGES+=("GPU Check")
[ "$ENABLE_REPORT_RESET" = true ] && ENABLED_STAGES+=("Report Reset")
[ "$ENABLE_RUST_INSTALL" = true ] && ENABLED_STAGES+=("Rust Install")
[ "$ENABLE_TOKENIZER_BUILD" = true ] && ENABLED_STAGES+=("Tokenizer Build")
[ "$ENABLE_TOKENIZER_TRAIN" = true ] && ENABLED_STAGES+=("Tokenizer Training")
[ "$ENABLE_TOKENIZER_EVAL" = true ] && ENABLED_STAGES+=("Tokenizer Evaluation")
[ "$ENABLE_DATASET_DOWNLOAD" = true ] && ENABLED_STAGES+=("Dataset Download")
[ "$ENABLE_BASE_TRAIN" = true ] && ENABLED_STAGES+=("Base Training")
[ "$ENABLE_BASE_LOSS" = true ] && ENABLED_STAGES+=("Base Loss Evaluation")
[ "$ENABLE_BASE_EVAL" = true ] && ENABLED_STAGES+=("Base CORE Evaluation")
[ "$ENABLE_IDENTITY_DOWNLOAD" = true ] && ENABLED_STAGES+=("Identity Download")
[ "$ENABLE_MID_TRAIN" = true ] && ENABLED_STAGES+=("Midtraining")
[ "$ENABLE_MID_EVAL" = true ] && ENABLED_STAGES+=("Midtraining Evaluation")
[ "$ENABLE_SFT_TRAIN" = true ] && ENABLED_STAGES+=("SFT Training")
[ "$ENABLE_SFT_EVAL" = true ] && ENABLED_STAGES+=("SFT Evaluation")
[ "$ENABLE_RL_TRAIN" = true ] && ENABLED_STAGES+=("RL Training")
[ "$ENABLE_RL_EVAL" = true ] && ENABLED_STAGES+=("RL Evaluation")
[ "$ENABLE_REPORT_GENERATE" = true ] && ENABLED_STAGES+=("Report Generation")

# Check if any stages are enabled
if [ ${#ENABLED_STAGES[@]} -eq 0 ]; then
    log ""
    log "⚠️  WARNING: No pipeline stages are enabled!"
    log ""
    log "All ENABLE_* flags are set to false. Nothing will be executed."
    log "Please edit this script and set one or more ENABLE_* flags to true."
    log ""
    log "Example stages you might want to enable:"
    log "  - ENABLE_BASE_TRAIN=true       # Train the base model"
    log "  - ENABLE_SFT_TRAIN=true        # Run supervised fine-tuning"
    log "  - ENABLE_REPORT_GENERATE=true  # Generate training report"
    log ""
    exit 1
else
    log "✓ Enabled stages (${#ENABLED_STAGES[@]}):"
...
# Detect VRAM and set SFT batch size if enabled
if [ "$AUTO_DETECT_SFT_VRAM" = true ]; then
    detect_vram_and_set_sft_batch_size
else
    log "SFT VRAM auto-detection disabled, using configured SFT_DEVICE_BATCH_SIZE=$SFT_DEVICE_BATCH_SIZE"
fi
